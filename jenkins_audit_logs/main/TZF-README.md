# TZF Jenkins — 1-Year Audit Archive & Dashboard Setup

---

## Step 1 — Install Jenkins Plugins (if not already installed)

Go to **Manage Jenkins → Plugins → Available** in the TZF Jenkins UI and install:
...
```bash
# Connect to TZF Jenkins via SSM
aws ssm start-session --target <INSTANCE_ID> --region ap-south-1 --profile tzfmain
...
#!/bin/bash
# TZF Jenkins Archive Configuration
...
S3_PREFIX="${S3_PREFIX:?S3_PREFIX must be set — use tzf for TZF}"
...
{
  echo "================================================================"
  echo "Jenkins Monthly Archive - $MONTH - TZF"
  echo "================================================================"
...
{
  echo "JENKINS 1-YEAR AUDIT COMPLIANCE REPORT — TZF"
  echo ""
...
```bash
# From your Mac terminal — SCP the dashboard file to tzf Jenkins
...
# Update the title to say TZF
sudo sed -i 's/<title>Jenkins Audit Dashboard<\/title>/<title>Jenkins Audit Dashboard - TZF<\/title>/' /var/lib/jenkins/jenkins-1year-archive/jenkins-audit-dashboard.html
...
[Unit]
Description=Jenkins Audit Dashboard API - TZF
...
> **Note:** On TZF, the jenkins user doesn't have crontab access (`/etc/cron.allow` restricts it). Use `sudo crontab -e` to add cron jobs under root instead.
...
The dashboard runs on port **9090**. Add an inbound rule to the TZF Jenkins EC2 Security Group:
...
curl -s http://localhost:9091/api/stats | python3 -m json.tool
curl -s http://localhost:9091/api/logs | python3 -m json.tool | head -20

# Test dashboard HTML loads
curl -s http://localhost:9091/jenkins-audit-dashboard.html | head -10
```

From your **Mac terminal** (not SSH):

```bash
# Replace with actual public IP of tzf Jenkins
curl -s https://jenkins-payments-audit.jinka.in/jenkins-audit-dashboard.html | head -10
```

Then open in browser:

```
https://jenkins-payments-audit.jinka.in/jenkins-audit-dashboard.html
```

> ⚠️ **Note:** Domain `jenkins-payments-audit.jinka.in` currently not loading — DNS or nginx/proxy setup pending on TZF Jenkins server.
...
Since the TZF Jenkins is in a **private subnet** (no public IP access), the audit-viewer script lets you inspect all audit data directly from the terminal without needing a web browser.
...
scp ubuntu@<JENKINS_SERVER_IP>:/tmp/jenkins-audit-TZF-*.csv ~/Downloads/
scp ubuntu@<JENKINS_SERVER_IP>:/tmp/jenkins-audit-TZF-*.json ~/Downloads/
```
...
**Option 1 — Direct URL (once DNS/nginx configured):**

```
https://jenkins-payments-audit.jinka.in/jenkins-audit-dashboard.html
```

> ⚠️ Currently not loading — pending DNS + nginx setup on TZF Jenkins.

**Option 1b — SSH Tunnel (workaround until domain works):**

```bash
# From your Mac terminal — create SSH tunnel
ssh -L 9091:localhost:9091 ubuntu@<JENKINS_SERVER_PUBLIC_IP>
```

Then open in browser:

```
http://localhost:9091/jenkins-audit-dashboard.html
```
...
aws ssm start-session \
  ...
  --parameters '{"portNumber":["9091"],"localPortNumber":["9091"]}' \
  --region ap-south-1 \
  --profile bbwalletmain
```
...
| Issue | Fix |
|-------|-----|
| Dashboard not loading | Check `curl https://jenkins-payments-audit.jinka.in/` — DNS + nginx/proxy must be configured first |
...
| Object | Path |
|--------|------|
| Per-day events | `s3://bb-jenkins-audit-logs/tzf/<YYYY>/<MM>/audit-<YYYY-MM-DD>.log.gz` |
| Latest full snapshot | `s3://bb-jenkins-audit-logs/tzf/audit-combined-latest.log.gz` |
...
Verify upload:
```bash
aws s3 ls s3://bb-jenkins-audit-logs/tzf/$(date -u +%Y)/$(date -u +%m)/ --profile tzfmain
```
...
If `bb-jenkins-audit-logs/bbfinserv/`, `/tzf/`, `/zinka-divum/` show **identical** files, one server has wrong `S3_PREFIX` hardcoded in `merge-audit-logs.sh` — fix by setting `S3_PREFIX="tzf"` on TZF EC2 and redeploying.
...
├── build-logs/                          # (Optional)
├── jenkins-audit-api.py                 # Python API server (port 9091)
...
> Cron jobs are under **root** (`sudo crontab -l`) since jenkins user doesn't have crontab access on TZF.
